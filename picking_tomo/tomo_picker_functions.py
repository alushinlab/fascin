	...
	for i in tqdm(range(0, real_data.shape[0]-box_size, increment_z)):
		for j in range(0, real_data.shape[1]-box_size, increment_y):
			for k in range(0, real_data.shape[2]-box_size, increment_x):
				...
				# rotate extraction by 90 degrees because training data was rotated wrong
				#extraction = np.rot90(extraction, k=1, axes=(1,2))
...
def run_pick_on_micrograph(file_name, INCREMENT, BOX_SIZE, hist_matcher, FUDGE_FAC, OUTPUT_DIR_PATH, DAE, FCN, THRESHOLD):
	...
	dae_stitch_back = stitch_back_seg(real_data.shape, dae_preds, INCREMENT, INCREMENT, INCREMENT, BOX_SIZE) #last INCREMENT is the z, I was testing with 4
	...
	fcn_preds = []
	for i in tqdm(range(0, extractions.shape[0], batch_size)):
		batch_pred = FCN.predict(extractions[i:i+batch_size])
	...
	stitch_back_channel_0 = stitch_back_seg(real_data.shape, fcn_preds_channel_0, INCREMENT, INCREMENT, INCREMENT, BOX_SIZE)
	stitch_back_channel_1 = stitch_back_seg(real_data.shape, fcn_preds_channel_1, INCREMENT, INCREMENT, INCREMENT, BOX_SIZE)
	stitch_back_channel_2 = stitch_back_seg(real_data.shape, fcn_preds_channel_2, INCREMENT, INCREMENT, INCREMENT, BOX_SIZE)
