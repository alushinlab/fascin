...
def generate_box_mask(output_filename, corners, overall_dims):
    # Define the three axes of the OBB, from the given corners (assumes corners are given in a specific order)
    axes = [
        corners[1] - corners[0],  # axis 0
        corners[2] - corners[0],  # axis 1
        corners[4] - corners[0],  # axis 2
    ]
    # Create an empty volume
    mask = np.zeros(overall_dims, dtype=np.uint8)
    # Check every point in the volume to see if it's inside the OBB
    for x in tqdm(range(overall_dims[0])):
        for y in tqdm(range(overall_dims[1])):
            for z in range(overall_dims[2]):
                point = np.array([x, y, z])
                if is_inside_OBB(point, corners):
                    mask[x, y, z] = 1
    # Save the mask as an MRC file
    with mrcfile.new(output_filename, overwrite=True) as mrc:
        mrc.set_data(mask.astype(np.int8))  # Ensure data type compatibility with MRC format
        mrc.voxel_size = 1  # Modify as needed based on the actual voxel size
...
for i in range(0, len(tomo_name_list)):
    ...
    WIDTH = 700
    p1 = (1579.266, 1766.522, 261.115)
    p2 = (1479.347, 3239.864, 370.585)
    ...
    box_biild_output_name = cmm_file_name_base + tomo_name_list[i]+'/paired_bilds/pruning_box.bild'
    generate_box_bild(box_biild_output_name, generate_OBB(p1,p2, 1000, WIDTH))
    generate_box_mask(box_biild_output_name[:-5]+'.mrc', generate_OBB(p1,p2, 1000, WIDTH), denoised_map.shape)
    ...
    norm = plt.Normalize(vmin=0, vmax=3)# 1 and 3 for weighted product
    ...
    for j in range(0, len(my_point_holder_plus_energy)):
        ...
        this_pairs_energy_score = avg_energy_score#num_fascin_bridges/computed_interface_length / (1+avg_energy_score)
        ...
        generate_bild(bild_output_name, my_point_holder_plus_energy[j],  this_pairs_color, avg_energy_score)
