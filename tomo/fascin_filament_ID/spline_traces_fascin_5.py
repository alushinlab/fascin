...
cmm_file_name_base = '/rugpfs/fs0/cem/store/mreynolds/fascin_tomos/subtomo_averaging/alpha_values_bin1/bilds/automated_fil_identification_onlyPC1InterfaceScore/'
bild_file_name_base = '/rugpfs/fs0/cem/store/mreynolds/fascin_tomos/subtomo_averaging/alpha_values_bin1/bilds/'
energyScores_file_name = '/rugpfs/fs0/cem/store/mreynolds/fascin_tomos/subtomo_averaging/alpha_values_bin1/bilds/filament_identification/allEnergyScores_PC1.txt'
...
#tomo_name_list = ['ts044_A', 'ts045_A', 'ts045_B', 'ts045_C', 'ts045_D', 'ts053_A', 'ts053_B', 'ts058_A', 'ts058_B','ts058_C','ts059_A','ts059_B','ts059_C','ts059_D','ts062_A','ts064_A','ts064_B','ts064_C','ts065_A','ts072_A','ts072_C','ts074_A','ts074_B','ts074_C']
...
for i in range(0, len(tomo_name_list)):
    ...
    #norm = plt.Normalize(vmin=-7000, vmax=7000)# use this for PC1only
    norm = plt.Normalize(vmin=0.0, vmax=7)# 1 and 3 for weighted product
    ...
    for j in range(0, len(my_point_holder_plus_energy)):
        ...
        #avg_energy_score = np.mean(my_point_holder_plus_energy[j][:,3]) # use this for raw PC values
        avg_energy_score_raw = np.mean(my_point_holder_plus_energy[j][:,3])
        avg_energy_score = np.mean(np.abs(my_point_holder_plus_energy[j][:,3])) # use this for computing interface scores
        ...
        this_pairs_energy_score = num_fascin_bridges/computed_interface_length * (1-avg_energy_score/10000)
        #this_pairs_energy_score = len(my_point_holder_plus_energy[j][:,3]) * np.exp(-1*np.mean(my_point_holder_plus_energy[j][:,3]))
        #this_pairs_energy_score = len(my_point_holder_plus_energy[j][:,3])
        ...
        energy_score_holder.append((avg_energy_score_raw, avg_energy_score, num_fascin_bridges, num_fascin_bridges/computed_interface_length, this_pairs_energy_score))

    energy_score_holder = np.asarray(energy_score_holder)
    np.savetxt(cmm_file_name_base + tomo_name_list[i]+'/energy_score_holder.csv', energy_score_holder, delimiter=',',fmt='%.3f')
